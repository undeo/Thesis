...
\subsection*{Shape}
The shape of the aTGC contribution is designed as sum of the different contributions. The normalization is taken from the scaling factor mentioned above, while the signal function is normalized to unity. The signal function consists of several parts and is finally given by
\begin{equation}
...
\begin{align}
F_{\rm aTGC} =& \frac{N_{\rm SM}}{A_N} e^{-a_{SM}x} + \sum_i \left( \frac{N_{c_i,1}}{A_N} c_i^2 e^{-a_{i,1} x} \frac{1+\rm{Erf}\left(\frac{x-a_{o,i}}{a_{w,i}}\right)}{2} + \frac{N_{c_i,2}}{A_N} c_i e^{-a_{i,2}x} \right)  ~,
