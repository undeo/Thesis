...
Instead of maximizing $L$ it is conventional to minimize $-L$. It is also convenient to use the logarithm of the likelihood function, which allows to add different likelihoods instead of multiplying them. In the following, $L$ refers to the logarithmic likelihood.
...
\end{equation}
The first derivative of the likelihood vanishes at $\hat{\theta}$ per definition. Using the Cram\'{e}r-Rao-bound and neglecting terms of higher order leads to
\begin{equation}
...
\end{equation}
where $k$ denotes the total number of observed events, $S(\boldsymbol{c})$ the expected signal yield depending on the POI $\boldsymbol{c}$, and $B$ the expected background event yield. $f_{\rm aTGC}(\boldsymbol{c})$ and $f_{\rm bkg}$ are given by the pdfs describing the signal and the background processes, respectively, while  $\rho(\boldsymbol{\theta})$ is the pdf describing the set of uncertainties $\boldsymbol{\theta}$. There are two kinds of uncertainties implemented as nuisance parameters $\boldsymbol{\theta}$. Uncertainties on the normalizations of the different contributions are implemented as log-normal distributions [???]
Uncertainties affecting the slope parameters of the background and signal estimation are implemented by adding a Gaussian term centred at the nominal parameter values and a width corresponding to the uncertainty. \\

Expected limits are obtained by creating a pseudo-dataset using only the background contributions.
...
The uncertainties mentioned above do also affect the shape of the signal distribution. The uncertainties on the slope parameters of the signal function were also not estimated as part of this thesis but taken from \cite{PAS}.\\
...
\noindent Another effect on the shapes of two-dimensional limits are the interfenrence effects between the different aTGC contributions introduced in section~\ref{}, which causes the contours to be slightly rotated counterclockwise. This is described in more detail in section~\ref{}.
...
The effects of the interfenrence between different aTGC contributions can be illustrated in terms of a simple counting experiment with two aTGC parameters $c_1$ and $c_2$, neglecting interference with the SM contribution. The number of observed events $N_{\rm obs}$ is then given by
\begin{equation}
N_{\rm obs} = A_{\rm SM} + c_1^2A_1^2 + c_2^2A_2^2 + c_1c_2A_1A_2 ~.
\end{equation}
